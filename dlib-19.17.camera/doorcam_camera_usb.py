...
def get_jetson_gstreamer_source(dev=0, width=640, height=480):
    """
...
# gst-launch-1.0 v4l2src device=/dev/video0 ! video/x-raw,format=YUY2,width=640, height=360, framerate=30/1 ! xvimagesink
# gst-launch-1.0 v4l2src device=/dev/video0 ! video/x-raw,format=YUY2,width=320, height=240, framerate=30/1 ! xvimagesink
# gst-launch-1.0 v4l2src device=/dev/video0 ! video/x-raw,format=YUY2,width=1280, height=720, framerate=15/1 ! xvimagesink
...
def lookup_known_face(face_encoding):
    ...
    if face_distances[best_match_index] < 0.5:
        ...
        # IACT_HEO minutes=5
        if datetime.now() - metadata["first_seen_this_interaction"] > timedelta(seconds=2):
...
def main_loop():
    ...
    while True:
        ...
        frame = Rotate(img, 180) #90 or 180 or 270
        ########################################
